 ...
 * prefix (e.g. BEX_) or program name (e.g. CFDISK_)
 *
 * In the code is possible to use
 *
 *	DBG(FOO, bex_debug("this is output for foo"));
 *
 ...
 * subsystem names (e.g. "LIBBEX_DEBUG=options,tab"). In this case is
 * necessary to define mask names array. This functionality is optional.
 *
 * It's stringly recommended to use BEX_* macros to define/declare/use
 * the debug stuff.
 */
...
struct bex_debug_maskname {
	const char *name;
	int mask;
	const char *help;
};
#define BEX_DEBUG_EMPTY_MASKNAMES {{ NULL, 0, NULL }}
#define BEX_DEBUG_DEFINE_MASKNAMES(m) static const struct bex_debug_maskname m ## _masknames[]
#define BEX_DEBUG_MASKNAMES(m)	m ## _masknames

#define BEX_DEBUG_DEFINE_MASK(m) int m ## _debug_mask
#define BEX_DEBUG_DECLARE_MASK(m) extern BEX_DEBUG_DEFINE_MASK(m)

/* p - flag prefix, m - flag postfix */
#define BEX_DEBUG_DEFINE_FLAG(p, m) p ## m

/* l - library name, p - flag prefix, m - flag postfix, x - function */
#define __BEX_DBG(l, p, m, x) \
	do { \
...
#define __BEX_INIT_DEBUG(lib, pref, mask, env) \
	do { \
		...
		; \
			...
			char *str = getenv(# env); \
			if (str) \
				lib ## _debug_mask = bex_debug_parse_envmask(lib ## _masknames, str); \
		} else \
...
bex_debugobj(void *handler, const char *mesg, ...)
{
	va_list ap;
...
static inline int bex_debug_parse_envmask(
			const struct bex_debug_maskname flagnames[],
			const char *mask)
{
	int res;
	char *ptr;
	...
	if (ptr && *ptr && flagnames && flagnames[0].name) {
		...
		while ((name = strtok_r(ms, ",", &ptr))) {
			const struct bex_debug_maskname *d;
			ms = ptr;
...
static inline void bex_debug_print_masks(
			const char *env,
			const struct bex_debug_maskname flagnames[])
{
	const struct bex_debug_maskname *d;
