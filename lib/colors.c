 ...
 */
static BEX_DEBUG_DEFINE_MASK(termcolors);
BEX_DEBUG_DEFINE_MASKNAMES(termcolors) = BEX_DEBUG_EMPTY_MASKNAMES;
...
#define DBG(m, x)       __BEX_DBG(termcolors, TERMCOLORS_DEBUG_, m, x)
#define ON_DBG(m, x)    __BEX_DBG_CALL(termcolors, TERMCOLORS_DEBUG_, m, x)
			      ...
			      int  *filetype)
{
	...
	if (strcmp(type_start, "disable") == 0)
		...
		*filetype = UL_COLORFILE_SCHEME;
		...
		DBG(CONF, bex_debug("unknown type '%s'", type_start));
...
static int colors_readdir(struct ul_color_ctl *cc, const char *dirname)
{
	...
	DBG(CONF, bex_debug("reading dir: '%s'", dirname));
	...
	while ((d = readdir(dir))) {
		...
		DBG(CONF, bex_debug("item '%s': score=%d "
		...
		DBG(CONF, bex_debug("setting '%s' from %d -to-> %d",
			     ...
			     char *seq0)
{
	...
	DBG(SCHEME, bex_debug("add '%s'", name));
	...
	if (isalpha(*seq)) {
		...
		if (!s) {
			DBG(SCHEME, bex_debug("unknown logical name: %s", seq));
			rc = -EINVAL;
			goto err;
		}
...
static void colors_free_schemes(struct ul_color_ctl *cc)
{
	...
	DBG(SCHEME, bex_debug("free scheme"));
...
static void colors_sort_schemes(struct ul_color_ctl *cc)
{
	...
	DBG(SCHEME, bex_debug("sort scheme"));
						 ...
						 const char *name)
{
	...
	DBG(SCHEME, bex_debug("search '%s'", name));
...
static int colors_read_schemes(struct ul_color_ctl *cc)
{
	...
	DBG(SCHEME, bex_debug("reading file '%s'", cc->sfile));
...
{
	__BEX_INIT_DEBUG(termcolors, TERMCOLORS_DEBUG_, 0, TERMINAL_COLORS_DEBUG);
}
...
#endif
	if (ncolors != -1) {
		DBG(CONF, bex_debug("terminal is ready (supports %d colors)", ncolors));
		return 1;
	}
none:
	DBG(CONF, bex_debug("terminal is NOT ready"));
	return 0;
}
